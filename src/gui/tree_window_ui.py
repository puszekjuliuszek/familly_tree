...
class TreeWindowUi(object):
    def setup_ui(self, MainWindow, person):
        ...
        while len(queue) > 0:
            ...
            if person_tmp.mother is not None:
                if level_tmp == level:
                    queue.append((person_tmp.mother, low_border, low_border + (width_tmp - COUPLE_GAP) / 2, level_tmp + 1,"GM"))
                else:
                    queue.append((person_tmp.mother,low_border,low_border+(width_tmp-REGULAR_GAP)/2 ,level_tmp+1,"M"))
    ...
    def add_paint_cords(self,gender,X,Y, low_border, high_border):
        if gender == "R":
            ...
            self.paint_cords.append((X+PERSON_WIDTH,int(Y+PERSON_HEIGHT/2),int(high_border+COUPLE_GAP/2),int(Y+PERSON_HEIGHT/2)))
        else:
